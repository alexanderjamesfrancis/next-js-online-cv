...
export default function JobCardList(data){
  ...
  const jobCard = description.map((jobItem, index) => 
  <CardBuilder role= {jobItem} key = {index} />   
)
            ...
            <div class="underline underline-offset-1 mb-4">
              <h3 class="font-bold">{data.companyName}</h3>
                <div class="font-semibold">
                  <h4>From {data.dateWorked}</h4>
                  <h3>{data.roleWorked}</h3>
                </div>
                
            </div>
              
              <p class="mb-4">{data.descriptionHeader}
